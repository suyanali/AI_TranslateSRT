...
# -*- coding: utf-8 -*-
# transcribe_pro_gui.py (v1.6 - Complete & Final)
...
RUN_AND_LOG_SCRIPT_NAME = "run_and_log.py"
TRANSCRIPTION_SCRIPT_NAME = "transcribe_pro_v5_banch_04_banch_09.py"
# SUMMARIZER_SCRIPT_NAME 已被整合，不再需要
...
class TranscriptionApp:
    def __init__(self, master):
        self.master = master
        self.master.title(f"字幕轉錄與日誌工具 v1.6 (啟動器: {RUN_AND_LOG_SCRIPT_NAME})")
...
Sherlock = 夏洛克""")
    ...
    def _check_dependencies(self):
        missing_files = []
        required_scripts = [RUN_AND_LOG_SCRIPT_NAME, TRANSCRIPTION_SCRIPT_NAME]
        for script in required_scripts:
            if not os.path.exists(resource_path(script)):
                missing_files.append(script)
        if missing_files:
            messagebox.showerror("依賴性錯誤", f"找不到必要的腳本檔案:\n\n" + "\n".join(missing_files) + "\n\n請確保所有腳本都和本程式放在同一個資料夾中，或在打包時已正確包含。")
            self.master.destroy()
            return
        if sys.platform == "win32" and not os.path.exists(resource_path("ffmpeg.exe")):
            self._log_message("警告: 在程式目錄中未找到 'ffmpeg.exe'。請確保它與主程式在同一資料夾，或在打包時已包含。\n")
    ...
    def _build_command(self):
        ...
        try:
            ...
            self.temp_prompt_file = f"temp_prompt_{datetime.now().strftime('%Y%m%d%H%M%S')}.md"
            with open(self.temp_prompt_file, 'w', encoding='utf-8') as f:
                f.write(prompt_content)
            self._log_message(f"資訊: 已生成臨時 Prompt 檔案: {self.temp_prompt_file}\n")
        except Exception as e:
            messagebox.showerror("錯誤", f"無法建立臨時 Prompt 檔案: {e}")
            return None
        
        command = [
            f'\"{sys.executable}\"',
            RUN_AND_LOG_SCRIPT_NAME,
            f'\"{input_file}\"',
            "--prompt_file", f'\"{os.path.abspath(self.temp_prompt_file)}\"',
    ...
    def _run_process(self, command):
        try:
            ...
            # *** 關鍵修正: 強制子程序使用 UTF-8 編碼處理標準輸入輸出 ***
            # 這可以從根本上解決在繁體中文 Windows 上打包成 exe 後的 'cp950' 編碼錯誤
            env["PYTHONUTF8"] = "1"

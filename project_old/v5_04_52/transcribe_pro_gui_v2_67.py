# transcribe_pro_gui_v2_67.py
# 版本號: v2_67_20250809
# 修改內容簡述:
# 1.  【功能恢復】: 再次從 v63 版本中完整恢復了 `_check_for_resume`, `_import_terms_from_txt`, 和 `_check_and_start_merge` 三個函式的彈窗邏輯，確保所有使用者選項都已還原。
# 2.  【錯誤修正】: 採用新的任務結束信號機制，徹底解決了在某些情況下「退出碼: None」的錯誤。
# 3.  【版本更新】: 指向最新的後端核心腳本 `transcribe_pro_v5_branch_04_branch_52.py`，並更新介面版本號至 v2.68。
# 4.  【架構】: 延續 `multiprocessing` 架構，從根本上解決打包後重啟的問題。
...
# 匯入重構後的後端任務模組
import transcribe_pro_v5_branch_04_branch_52 as backend_task
...
#  全域設定與常數
# ==============================================================================
CORE_SCRIPT_NAME = "transcribe_pro_v5_branch_04_branch_52.py"
...
{final_instruction}
'''
FIFTH_PRIORITY_TEMPLATE = """第五優先：不需要(音樂)(歌曲)這種備註，有對白或歌詞都需提取成srt"""
...
FINAL_INSTRUCTION_TEMPLATE = """最終指令：請嚴格按照以上所有規則，開始進行轉錄並生成符合規範的 SRT 檔案。"""
...
class TranscriptionApp:
    def __init__(self, master):
        ...
        self.master.title(f"AI 字幕轉錄工具 v2.67 (核心: {CORE_SCRIPT_NAME})")
    ...
    def _create_widgets(self):
        ...
        self.report_check.grid(row=3, column=0, columnspan=4, sticky="w", padx=5, pady=2)
        ...
        self.import_button = ttk.Button(config_frame, text="匯入設定檔 (.json)", command=self._import_settings)
        self.import_button.pack(side=tk.RIGHT, padx=2)
        self.export_button = ttk.Button(config_frame, text="匯出設定檔 (.json)", command=self._export_settings)
    ...
    def _set_ui_state(self, state):
        widgets_to_toggle = [self.browse_button, self.language_entry, self.add_term_button, self.edit_term_button, self.remove_term_button, self.api_key_entry, self.model_name_entry, self.chunk_duration_entry, self.temp_dir_entry, self.correction_threshold_entry, self.overlap_tolerance_entry, self.report_check, self.start_button, self.merge_button, self.import_button, self.export_button, self.main_rules_text, self.import_terms_button, self.export_terms_button]
    ...
    def _bind_settings_changes(self):
        for var in [self.api_key_var, self.model_name_var, self.chunk_duration_var, self.temp_dir_var, self.correction_threshold_var, self.overlap_tolerance_var, self.language_var, self.enable_report_var]:
    ...
    def _build_full_prompt(self):
        base_prompt = self.main_rules_text.get("1.0", tk.END).strip().replace("{terms_section}", "{sixth_priority}")
        terms = []
        for child in self.terms_tree.get_children():
        ...
        full_prompt = full_prompt.replace("{final_instruction}", FINAL_INSTRUCTION_TEMPLATE)
        return full_prompt
    ...
    def _run_process(self, config, is_summary_task=False):
        try:
            target_func = backend_task.run_summarize_only_task if is_summary_task else backend_task.run_transcription_task
                ...
                if isinstance(item, str):
                    ...
                    if not self.is_running:
                        if self.last_exit_code != 0:
                            ...
                            final_message = "任務已結束。"
                            if self.transcription_actually_performed:
    ...
    def _ensure_parameter_entries_editable(self):
        parameter_entries = [self.api_key_entry, self.model_name_entry, self.chunk_duration_entry, self.temp_dir_entry, self.correction_threshold_entry, self.overlap_tolerance_entry, self.language_entry]

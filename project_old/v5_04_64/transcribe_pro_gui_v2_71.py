# transcribe_pro_gui_v2_71.py
# 版本號: v2_71_20250810
# 修改內容簡述:
# 1.  【功能新增】: 在「進階設定」中新增「單行字數上限」輸入框，允許使用者自訂 prompt 中的 {max_chars}。
# 2.  【UI優化】: 為新的輸入框增加了滑鼠懸停提示 (Tooltip) 功能。
# 3.  【輸入驗證】: 限制「單行字數上限」輸入框只能輸入數字。
# 4.  【設定整合】: 新的字數上限設定會儲存於 config.json 中。
# 5.  【版本更新】: 更新介面版本號至 v2.71。
...
# 匯入重構後的後端任務模組
import transcribe_pro_v5_branch_04_branch_64 as backend_task
...
#  全域設定與常數
# ==============================================================================
CORE_SCRIPT_NAME = "transcribe_pro_v5_branch_04_branch_64.py"
...
class TranscriptionApp:
    def __init__(self, master):
        ...
        self.master.title(f"AI 字幕轉錄工具 v2.71 (核心: {CORE_SCRIPT_NAME})")
    ...
    def _bind_settings_changes(self):
        for var in [self.api_key_var, self.model_name_var, self.chunk_duration_var, self.temp_dir_var, self.correction_threshold_var, self.overlap_tolerance_var, self.language_var, self.max_chars_var, self.enable_report_var, self.keep_prompt_var]:

...
# 6.  【版本更新】: 更新介面版本號至 v2.74。
# 7.   介面佈局變更
# 8.  GUI 更新：將「結尾空白閾值」選項加入 GUI 面板，並修正了相關的程式碼。重疊容忍 (`overlap_tolerance`): 填寫 -1 可以關閉。結尾空白閾值 (`truncation_threshold`): 填寫 0 可以關閉
...
# 匯入重構後的後端任務模組
import transcribe_pro_v5_branch_04_branch_66 as backend_task
...
def process_wrapper(target_func, config, log_queue):
    ...
    try:
        ...
        # 如果函式沒有回傳值 (例如，因為 SystemExit)，則預設為 1
        sys.exit(exit_code if isinstance(exit_code, int) else 1)
    except SystemExit as e:
        # 捕捉由 sys.exit() 引發的 SystemExit 異常
        sys.exit(e.code)
    except Exception as e:
        # 捕捉目標函式本身可能引發的任何其他未處理異常
        if log_queue:
            # 記錄致命錯誤，以便主 GUI 能夠顯示
...
#  全域設定與常數
# ==============================================================================
CORE_SCRIPT_NAME = "transcribe_pro_v5_branch_04_branch_66.py"
...
class TranscriptionApp:
    def __init__(self, master):
        ...
        self.master.title(f"AI 字幕轉錄工具 v2.74 (核心: {CORE_SCRIPT_NAME})")
    ...
    def _create_widgets(self):
        ...
        CreateToolTip(self.main_rules_text, "請勿刪除或修改大括號 {} 內的佔位符（如 {language}、{max_chars}、{fifth_priority}、{sixth_priority}、{seventh_priority}、{final_instruction}），否則可能導致程式無法正常運作。\n大括號以外的文字可自由增減，但新增內容請添加於{final_instruction}之前")
        ...
        tooltip_text = "最終字幕的語言。例如輸入繁體中文會將字幕翻譯成繁體中文"
        CreateToolTip(self.language_entry, tooltip_text)
        ...
        self.correction_threshold_var = tk.StringVar(value="6") #"觸發自動重跑的嚴重修正次數閾值。"
        self.overlap_tolerance_var = tk.StringVar(value="0.5") #"允許的字幕時間軸重疊容忍秒數。"
        self.truncation_threshold_var = tk.StringVar(value="60") # "可疑的結尾空白秒數閾值"
        ...
        ttk.Label(params_frame, text="結尾空白閾值 (秒):").grid(row=3, column=0, sticky="w", padx=5, pady=2)
        self.truncation_threshold_entry = ttk.Entry(params_frame, textvariable=self.truncation_threshold_var, width=15)
        self.truncation_threshold_entry.grid(row=3, column=1, sticky="w", padx=5, pady=2)
        ...
        self.report_check.grid(row=4, column=0, columnspan=2, sticky="w", padx=5, pady=2)
    ...
    def _set_ui_state(self, state):
        widgets_to_toggle = [self.browse_button, self.language_entry, self.max_chars_entry, self.add_term_button, self.edit_term_button, self.remove_term_button, self.api_key_entry, self.model_name_entry, self.chunk_duration_entry, self.temp_dir_entry, self.correction_threshold_entry, self.overlap_tolerance_entry, self.truncation_threshold_entry, self.report_check, self.keep_prompt_check, self.start_button, self.merge_button, self.import_button, self.export_button, self.main_rules_text, self.import_terms_button, self.export_terms_button]
    ...
    def _bind_settings_changes(self):
        for var in [self.api_key_var, self.model_name_var, self.chunk_duration_var, self.temp_dir_var, self.correction_threshold_var, self.overlap_tolerance_var, self.truncation_threshold_var, self.language_var, self.max_chars_var, self.enable_report_var, self.keep_prompt_var, self.keep_partial_audio_var]:
    ...
    def _build_config_object(self, resume=False, recreate=False, merge_only=False, summarize_only=False, log_file=None):
        ...
        config.truncation_threshold = int(self.truncation_threshold_var.get())
    ...
    def _import_settings(self):
        ...
        try:
            ...
            self.truncation_threshold_var.set(data.get("truncation_threshold", "60"))

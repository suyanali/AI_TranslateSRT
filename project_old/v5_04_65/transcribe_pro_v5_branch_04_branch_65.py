# transcribe_pro_v5_branch_04_branch_65.py
# 版本號: v5_5_65_20250811
# 修改內容簡述:
# 1. 【功能新增】: 新增 `run_partial_transcription_task` 函式，實現對音訊指定時間區段進行局部轉錄的功能。
# 2. 【功能新增】: 新增 `parse_time_v10` 時間解析函式，取代舊版 v9，能夠精準處理到毫秒 (HH:MM:SS,ms)。
# 3. 【核心邏輯】: `run_partial_transcription_task` 會：
...
# 4. 【CLI 擴充】: `main_cli` 新增 `--partial_only`, `--start_time`, `--end_time`, `--keep_partial_audio` 等命令列參數以支援新功能。
# 5. 【版本更新】: 版本號更新至 v5.5.65。
# 6. 【功能變更】腳本的重做機制會更加嚴格，當偵測到字幕持續時間異常過長時，也會將其視為嚴重錯誤並觸發重試，默認修正閾值(「嚴重修正」)改為6。嚴重修正標準：無法解析時間戳或時間倒流、時間軸重疊、字幕持續時間異常過長(預設18秒[MAX_DURATION = timedelta(seconds=18)])
...
def main_cli():
    ...
    parser.add_argument("--model_name", default="models/gemini-1.5-pro-latest", help="要使用的 Gemini 模型名稱。")
